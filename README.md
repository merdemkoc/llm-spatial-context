...
**Both API keys live in `server/`.** That is the whole reason this repo has a backend — four Hono routes, `/api/observe`, `/api/suggest`, `/api/reflect` and `/api/speak`, with the prompts and the model choice server-side so the persona can be tuned without shipping anything to the browser. Every route **fails safe**: a malformed body, a missing key or a 400 from the SDK all degrade to that agent's own safe answer — silence, a decline, an empty reflection — rather than an error at the user.
