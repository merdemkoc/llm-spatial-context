...
| `companionState.ts` | Module-scope tldraw atoms — the three switches, which job the companion is on (`observing` / `composing`), the transcript, the remark currently being spoken, the notes it is about, the backlog behind it, the pause it has settled into, and the standing understanding                                        | `observationEnabled`, `voiceEnabled`, `followEnabled`, `companionStage`, `companionTranscript`, `companionUtterance`, `companionFocus`, `companionQueue`, `cancelThought`, `companionPacing`, `Pacing`, `boardUnderstanding` |
