...
| Server      | Hono 4 on `@hono/node-server` — five routes, only to hold the API keys            |
...
`src/companion/` imports `@/domain` and nothing from `src/canvas/` — everything it needs from the
canvas, reading (`readEpisodeContext`, `readBoardSummary`) or **writing** (`applyGrouping`,
`createAgentNotes`, `createAgentRelations`), is **injected** by `Canvas.tsx` as a function, not
imported. That is what keeps the whole loop — write-back included — testable without an editor: the
companion calls adapter code through a seam, it never becomes canvas state the companion holds.
...
| `understandingDrift.ts` | How far the board has drifted from the standing understanding — the free, local gate that decides when a re-derivation is worth paying for    | `driftOf`, `driftWeight`, `DRIFT_THRESHOLD`                                                                                                                                                                         |
| `boardSummary.ts`       | The whole board, folded for a model — capped node/pair counts, real clusters, strong proximity pairs, relations, top effective-strength pairs | `buildBoardSummary`, `BoardSummary`, `BoardNodeSummary`, `BoardCluster`, `BoardProximity`, `BoardRelationSummary`, `BoardEffectivePair`, `BOARD_NODE_LIMIT`, `BOARD_PROXIMITY_LIMIT`, `CLUSTER_INFLUENCE_THRESHOLD` |
| `clusterLayout.ts`      | Where a grouping's members go — packed into a grid around their shared centre, then slid clear of everything else. Pure, no tldraw            | `computeClusterLayout`, `ClusterPlacement`, `ClusterLayoutOptions`, `DEFAULT_CLUSTER_GAP`, `DEFAULT_CLUSTER_MARGIN`, `DEFAULT_CLUSTER_ITERATIONS`                                                                   |
| `ideaPlacement.ts`      | Top-left positions for new notes, in a column beside the existing board — a reflection's proposed ideas never carry a position of their own   | `placeNewNotes`, `IdeaPlacementOptions`, `DEFAULT_IDEA_GAP`                                                                                                                                                         |
...
| `relations.ts`        | Relation ⇄ arrow projection; reads bound arrows into `Relation`s, rebuilds them on import, owns gravity, draws a reflection's accepted arrows | `getCanvasRelations`, `createRelations`, `createAgentRelations`, `isRelationArrow`, `relationType`, `relationGravity`, `setRelationGravity`, `selectedRelationArrowIds`, `RELATION_META_KEY`, `RELATION_GRAVITY_META_KEY`, `ARROW_SHAPE_TYPE` |
...
| `boardContext.ts`     | The whole board, read for the companion — the one line feeding `buildBoardSummary` the live canvas, for the suggester and the reflection      | `readBoardSummary`                                                                                                                                                                                                                            |
| `ideas.ts`            | Reflected ideas, turned into ghosts and then into notes — the client places, the model never does                                             | `planIdeaNotes`, `createAgentNotes`                                                                                                                                                                                                           |
| `grouping.ts`         | A grouping, turned into moves — resolves the model's chosen ids against the live canvas, commits the planned targets as one undo step         | `planGrouping`, `applyGrouping`, `GroupingPlan`                                                                                                                                                                                               |
...
| `postItAppearance.ts` | An agent-authored note's border — heavier, in the agent's own ink. Pure, kept out of `PostItShapeUtil` so it's testable without a live editor | `isAgentAuthored`, `postItBorder`                                                                               |
...
| `CanvasAiActions.tsx`          | The companion's on-demand actions beside the toolbar — ✦ Suggest a grouping, ✦ Reflect on board — each asks first            | `CanvasAiActions`                                                                                                   |
| `CanvasControls.tsx`           | Composes `GroupingControls` + `IdeaControls` into tldraw's one `InFrontOfTheCanvas` slot; the two never coexist              | `CanvasControls`                                                                                                    |
| `GroupingControls.tsx`         | Accept/dismiss a pending grouping, floating above the proposed cluster; accept calls the companion's published handle        | `GroupingControls`                                                                                                  |
| `IdeaControls.tsx`             | Accept/dismiss a reflection's proposed notes and arrows, one at a time or all at once                                        | `IdeaControls`                                                                                                      |
...
| `CanvasOverlays.tsx`           | Composes every `OnTheCanvas` layer — fields, the grouping/idea/relation ghosts, the focus spotlight — into tldraw's one slot | `CanvasOverlays`                                                                                                    |
...
| `GroupingGhostOverlay.tsx`     | A pending grouping, previewed — each member highlighted where it is, a faint dashed ghost where it would land                | `GroupingGhostOverlay`                                                                                              |
| `IdeaGhostOverlay.tsx`         | A reflection's proposed notes, previewed as faint agent-tinted cards at their planned position                               | `IdeaGhostOverlay`                                                                                                  |
| `RelationGhostOverlay.tsx`     | A reflection's proposed arrows, previewed as dashed agent-tinted lines; one page-coordinate SVG holds every one              | `RelationGhostOverlay`                                                                                              |
...
| `companionState.ts` | Module-scope tldraw atoms — the three switches, which job the companion is on (`observing` / `composing`), the transcript, the remark currently being spoken, the notes it is about, the backlog behind it, the pause it has settled into, the standing understanding, and the pending grouping/idea/relation proposals plus the commit handles the UI calls to accept them | `observationEnabled`, `voiceEnabled`, `followEnabled`, `companionStage`, `companionTranscript`, `companionUtterance`, `companionFocus`, `companionQueue`, `cancelThought`, `companionPacing`, `Pacing`, `boardUnderstanding`, `groupingSuggestion`, `requestGrouping`, `acceptGrouping`, `GroupingSuggestion`, `ideaSuggestions`, `relationSuggestions`, `GhostIdea`, `GhostRelation`, `requestReflection`, `commitIdeas`, `commitRelations` |
...
| `suggestClient.ts`  | The seam to the suggester: POST the board and an intent (or the proactive trigger), receive which ids to cluster and why                                                                                                                                                                                                                                                    | `SuggestClient`, `createHttpSuggestClient`, `SUGGEST_TIMEOUT_MS`, `GroupingProposal`, `SuggestRequest`                                                                                                                                                                                                                                                                                                                                       |
| `reflectClient.ts`  | The seam to the reflection: POST the board and a persona, receive a reading plus notes and arrows to add                                                                                                                                                                                                                                                                    | `ReflectClient`, `createHttpReflectClient`, `REFLECT_TIMEOUT_MS`, `Reflection`, `ReflectRequest`, `IdeaProposal`, `RelationProposal`                                                                                                                                                                                                                                                                                                         |
...
| `digestClient.ts`   | The seam to the digest: POST the whole board, receive the standing understanding — kept, not spoken, so a partial reply is filled from `EMPTY_UNDERSTANDING` rather than left for a consumer to trip over                                                                                                                                                                   | `DigestClient`, `createHttpDigestClient`, `DIGEST_TIMEOUT_MS`, `BoardUnderstanding`, `Theme`, `DigestRequest`, `EMPTY_UNDERSTANDING`, `isBlankUnderstanding`                                                                                                                                                                                                                                                                                 |
...
Four agents that once each carried their own copy of the asking now share it:
...
| `prompting/boardRender.ts`    | A board written for a model — one `named()` where there were three, and the block format the suggester, the reflection and the digest share                                             | `named`, `namedList`, `realClusters`, `proximityPairs`, `renderBoardBlocks`, `renderRecentComments`, `boardLabels` |
| `prompting/fragments.ts`      | Prompt paragraphs more than one agent needs, so a missing rule looks missing                                                                                                            | `CANVAS_PRIMER`, `UNDERSTANDING_TRIAGE`                                                                            |
...
| `prompting/types.ts`          | The payload shapes the browser POSTs                                                                                                                                                    | `BoardSummaryPayload`, `RelationContext`, `BoardUnderstanding`, `Theme`                                            |
...
| Layer              | Files                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Env     |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Pure               | `domain/{canvas,spatialInfluence,effectiveStrength,canvasDiff,events,eventStream,episode,idleBackoff,understandingDrift,boardSummary,clusterLayout,ideaPlacement}.test.ts`, `companion/{renderEpisode,renderSuggest,renderReflect,renderDigest,reveal,thoughtQueue,digestClient}.test.ts`, `adapter/adapter.test.ts`, `adapter/relations.test.ts`, `grounding/{projection,grounding,visualId,annotationLayer,arrowAware}.test.ts`, `shapes/postItAppearance.test.ts` | `node`  |
| Real editor        | `adapter/editor.test.ts`, `adapter/relationEditor.test.ts`, `adapter/spatialEvents.test.ts`, `adapter/episodeValidity.test.ts`, `adapter/grouping.test.ts`, `adapter/ideas.test.ts`, `dev/seedScenario.test.ts`, `grounding/groundedExport.test.ts`, `companion/{companion,voiceClient}.test.ts`                                                                                                                                                                     | `jsdom` |
| Rendered component | `ui/ContextualFieldControl.test.tsx`, `ui/RelationGravityControl.test.tsx`, `ui/ContextualFieldOverlay.test.tsx`, `ui/InfluenceBadges.test.tsx`, `ui/EventLogPanel.test.tsx`, `ui/Companion*.test.tsx`, `ui/AgentThinkingIndicator.test.tsx`, `ui/CanvasAiActions.test.tsx`, `ui/GroupingControls.test.tsx`, `ui/IdeaControls.test.tsx`, `ui/GroupingGhostOverlay.test.tsx`, `ui/IdeaGhostOverlay.test.tsx`, `ui/RelationGhostOverlay.test.tsx`                      | `jsdom` |
...
- **The companion holds no canvas state.** It reads the document only through `readEpisodeContext`
  (an episode's ids → note text) and `readBoardSummary` (the whole board, for the suggester and the
  reflection), and writes to it only through `applyGrouping` / `createAgentNotes` /
  `createAgentRelations` — all four are functions injected by `Canvas.tsx`, never imports. Everything
  else it knows comes from the events. That is what makes `createCompanion` testable with a fake
  stream, fake clients and no editor at all, write-back included.
